...
check_token() {
  echo -e ""
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]               LICENSY ALBY DEVELOPER           [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e ""
  ...
  if echo "$VALID_TOKENS" | grep -wq "$USER_TOKEN"; then
    ...
    echo -e "${YELLOW}TELEGRAM : @albyy0x${NC}"
    echo -e "${YELLOW}WHATSAPP : 6283190118851${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 10K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}©albydeveloper${NC}"
    exit 1
  fi
  clear
}
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer AlbyDev Private   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${WHITE}ggg:     gg.     -   ,ggg       • WhatsApp : 6283190118851    ${NC}"
  echo -e "${WHITE} ggg:     ggg._    _,ggg        • Credit  : albyy0x ${NC}"
  echo -e "${WHITE} ggg.    '.'''ggggggp           • Support by Alby Botz  ${NC}"
